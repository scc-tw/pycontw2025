"""
PyO3 FFI benchmark implementations aligned with ctypes baseline.

This module provides PyO3-based implementations of all benchmark functions
...
try:
    import sys
    pyo3_dir = str(Path(__file__).parent)
    if pyo3_dir not in sys.path:
        sys.path.insert(0, pyo3_dir)
    
    import benchlib_pyo3
except ImportError as e:
    raise RuntimeError(f"Could not load PyO3 library: {e}")


class PyO3Benchmark:
    """PyO3 implementation of FFI benchmarks (aligned with ctypes)."""
    
    def __init__(self):
        """Initialize the PyO3 library wrapper."""
        self.lib = benchlib_pyo3
    ...
    def string_concat_operations(self, a: str = "hello", b: str = " world"):
        ...
        # PyO3 handles memory management automatically and returns Python string
    ...
    def array_operations_readonly(self, size: int = 1000):
        ...
        # Call function - PyO3 accepts Python lists and automatically passes size
    ...
    def array_operations_inplace(self, size: int = 1000, factor: float = 2.0):
        ...
        # Modify in place - PyO3 will handle the conversion and return modified array
        modified_data = self.lib.scale_doubles_inplace(data, factor)
        
        # Return first few elements for verification
        return modified_data[:min(5, size)]
    ...
    def structure_operations_create(self, x: int = 10, y: int = 20, value: float = 3.14):
        ...
        struct = self.lib.create_simple(x, y, value)
        # Access struct fields (PyO3 exposes them as properties)
    ...
    def callback_operations_simple(self, value: int = 42):
        ...
        # PyO3 can accept Python functions as callbacks directly
    ...
    def callback_operations_array(self, size: int = 100):
        ...
        return self.lib.sum_with_transform(data, size, transform)
    ...
    def compute_operations_matrix_multiply(self, m: int = 10, n: int = 10, k: int = 10):
        ...
        result_c_data = self.lib.matrix_multiply_naive(a_data, b_data, c_data, m, n, k)
        
        # Return checksum for verification
        return sum(result_c_data)
...
def create_pyo3_benchmark() -> PyO3Benchmark:
    """Factory function to create PyO3 benchmark instance."""
...
if __name__ == "__main__":
    # Self-test
    print("Testing PyO3 benchmark implementation (aligned with ctypes)...")
    
    bench = create_pyo3_benchmark()
